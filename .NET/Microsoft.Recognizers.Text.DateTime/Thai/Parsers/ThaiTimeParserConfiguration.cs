...
{
    public class ThaiTimeParserConfiguration : BaseOptionsConfiguration, ITimeParserConfiguration
    {
        ...
        public ThaiTimeParserConfiguration(ICommonDateTimeParserConfiguration config) : base(config.Options)
        {
            TimeTokenPrefix = DateTimeDefinitions.TimeTokenPrefix;
            AtRegex = ThaiTimeExtractorConfiguration.AtRegex;
            TimeRegexes = ThaiTimeExtractorConfiguration.TimeRegexList;
        ...
        public void AdjustByPrefix(string prefix, ref int hour, ref int min, ref bool hasMin)
        {
            ...
            //half
            if (trimedPrefix.StartsWith("ครึ่ง"))
            {
                deltaMin = 30;
            }
            // quarter
            else if (trimedPrefix.StartsWith("หนึ่งในสี่") || trimedPrefix.StartsWith("ควอร์เตอร์") || trimedPrefix.StartsWith("ควอเตอร์"))
            {
                deltaMin = 15;
            }
            // three quarters
            else if (trimedPrefix.StartsWith("สามในสี่") || trimedPrefix.StartsWith("ควอร์เตอร์") || trimedPrefix.StartsWith("ควอเตอร์"))
            {
                deltaMin = 45;
            }
            else
            {
                var match = ThaiTimeExtractorConfiguration.LessThanOneHour.Match(trimedPrefix);
            ...
            //to
            if (trimedPrefix.Contains("ถึง") || trimedPrefix.Contains("จนถึง"))
            {
                deltaMin = -deltaMin;
            }
